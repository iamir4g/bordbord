...
export function CommentForm({
  ...
  const [publisherSlug, setPublisherSlug] = React.useState("");
  const [ratings, setRatings] = React.useState<RatingElements>(defaultRatings);
  const [submitting, setSubmitting] = React.useState(false);
  const [success, setSuccess] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const contentLength = content.trim().length;
  const remainingCharacters = Math.max(0, 100 - contentLength);
  const publisherOptions = publishers
    .map((publisher) => ({
      name: typeof publisher.name === "string" ? publisher.name : "",
      slug: typeof publisher.slug === "string" ? publisher.slug : "",
    }))
    .filter((publisher) => publisher.name && publisher.slug)
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!publisherSlug) {
      setError("انتخاب ناشر برای ثبت دیدگاه الزامی است.");
      return;
    }
    if (trimmed.length < 100) {
      setError("متن دیدگاه باید حداقل ۱۰۰ کاراکتر باشد.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div>
        ...
        >
          ناشر مرتبط با این دیدگاه
        </label>
        <select
          id="publisher-select"
          value={publisherSlug}
          onChange={(e) => setPublisherSlug(e.target.value)}
          disabled={submitting || publisherOptions.length === 0}
          className="w-full rounded-xl border border-slate-850 bg-slate-950 p-3 text-xs text-slate-200 outline-none focus:border-amber-500 disabled:cursor-not-allowed disabled:opacity-50"
        >
          <option value="">انتخاب ناشر الزامی است</option>
        ...
        <p className="mt-1.5 text-[11px] text-slate-500">
          این لیست از ناشرهای ثبت‌شده در بک‌اند بارگذاری می‌شود.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="mt-1.5 flex items-center justify-between text-[11px]">
          <span className="text-slate-500">حداقل ۱۰۰ کاراکتر برای ثبت نظر لازم است.</span>
          <span
            className={
              remainingCharacters === 0 ? "text-emerald-400" : "text-amber-400"
            }
          >
            {remainingCharacters === 0
              ? "حداقل کاراکتر تکمیل شد"
              : `${remainingCharacters} کاراکتر دیگر لازم است`}
          </span>
        </div>
      </div>
      ...
      <button
        type="submit"
        disabled={
          submitting || contentLength < 100 || publisherSlug.length === 0
        }
